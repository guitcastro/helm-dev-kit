...
func main() {
	if len(os.Args) < 3 {
		fmt.Fprintf(os.Stderr, "Usage: %s <input.hcl> <output-dir> [chart-name]\n", os.Args[0])
		os.Exit(1)
	}
	...
	chart, err := conv.ConvertFile(inputFile)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error converting HCL to Helm: %v\n", err)
		os.Exit(1)
	}
...
func writeChart(chart *helm.Chart, outputDir string) error {
	...
	if err := ioutil.WriteFile(chartYAMLPath, []byte(chartYAML), 0644); err != nil {
	...
	if err := ioutil.WriteFile(valuesYAMLPath, []byte(valuesYAML), 0644); err != nil {
	...
	for _, template := range chart.Templates {
		...
		if err := ioutil.WriteFile(templatePath, []byte(template.Content), 0644); err != nil {
